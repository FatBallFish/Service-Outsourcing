...
class MessagesAdmin(admin.ModelAdmin):
    fieldsets = (
        ("基本信息", {'fields': ('id', 'sendID', 'recID', 'add_time', 'update_time')}),
    ...
    # 列表页显示内容
    list_display = ('id', 'sendID', 'recID', 'text', "status", 'add_time')
    list_display_links = list_display  # 列表中可点击跳转的字段

    search_fields = ('sendID__username', 'recID__username', 'text__title', 'text__content')  # 列表搜索字段
    list_filter = ('id', 'sendID__username', 'recID__username', 'status')  # 列表筛选字段
    list_per_page = 10  # 列表每页最大显示数量，默认100
...
class MessageSysStatusAdmin(admin.ModelAdmin):
    fieldsets = (
        ...
        ("消息内容", {'fields': (
            'message', 'message_id', 'text_title', 'text_content', 'message_add_time', 'message_update_time')}),
        ("读取状态", {'fields': ('recID', 'status',)})
    )
    readonly_fields = (
    'id', 'message_id', 'text_title', 'text_content', 'message_add_time', 'message_update_time')  # 自定义的数据类型只能设置为只读状态
    # 列表页显示内容
    list_display = ('id', 'message', "status", 'add_time')
    list_display_links = list_display  # 列表中可点击跳转的字段

    search_fields = ('message__text__title', 'message__title__content')  # 列表搜索字段
    list_filter = ('id', 'message__text__title', 'message__text__content', 'status')  # 列表筛选字段
    list_per_page = 10  # 列表每页最大显示数量，默认100
